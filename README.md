...
- 内壁は屋根で切らず階高 `H` のまま立てる。軒の近くでは屋根の上面が `H` より下がるので、外壁に取り付く内壁の端が屋根を 勾配 × 外壁厚/2 だけ突き抜ける（勾配 4 寸・外壁厚 150 mm で 30 mm）。屋根で切るのは外壁だけという方針（設計書 §8.6）を保つための意図的な残存。直すなら §8.6 の対象を内壁に広げる判断が先に要る
